...
namespace Tharga.InfluxCapacitor.Collector.Handlers
{
    public class DataInitiator
    {
        ...
        private Tuple<string, OutputLevel> CreateProcessorCounter()
        {
            ...
            var counters = new List<ICounter> { new Counter("Processor", "% Processor Time", "*", null, null, null, null) };
        ...
        private Tuple<string, OutputLevel> CreateMemoryCounter()
        {
            ...
            var counters = new List<ICounter> { new Counter("Memory", "*", string.Empty, null, null, null, null) };
            var response = new CounterGroup(name, 10, 0, counters, new ITag[] { }, CollectorEngineType.Safe);
            return ConvertErrorsToWarnings(CreateFile(name, response));
        }
